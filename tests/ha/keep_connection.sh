...
set -e
grep "Received request to set keep host connected indication for host ${NQN}host4 on subsystem ${NQN}, context: None" /var/log/ceph/nvmeof-$GW2_NAME/nvmeof-log
set +e
    grep "Received request to remove host ${NQN}host4 access from ${NQN}," /var/log/ceph/nvmeof-$GW2_NAME/nvmeof-log | grep "context: None"
    if [[ $? -eq 0 ]]; then
        echo "Shouldn't get a remove host command in update"
        exit 1
    fi
set -e
